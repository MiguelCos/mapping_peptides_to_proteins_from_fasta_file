...
# Miguel Cosenza v 0.3 

annotate_peptides <- function(peptide_sequence, fasta){ # input should be a vector of peptide sequences and the fasta file
   ...
   list_elem <- purrr::keep(fasta,
                            function(x){str_detect(x, 
                                                   pattern = peptide_sequence[i])})
   ...
   if(is_empty(list_elem)){
                       ...
                       print(paste(i, "out of", length(peptide_sequence)))
   }
   }
